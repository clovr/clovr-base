...
if($samtools eq "" && defined($ENV{SAMTOOLS_HOME})) {
...
##
# Ensure that we can run a 'samtools' binary.
#
sub ensureSamtools {
	$samtools = $samtools_arg if $samtools_arg ne "";
	unless(-x $samtools) {
		if($samtools_arg ne "") {
			die "Error: -samtools parameter specifies a non-existant or non-executable file \"$samtools_arg\"\n";
		} else {
			die "Error: Could not find samtools in \$SAMTOOLS_HOME or \$PATH and -samtools was not specified\n";
		}
	}
	my $so = `$samtools 2>&1`;
	print STDERR $so;
	$so =~ /Usage/ || die "Could not execute $samtools\n";
}
...
if ($push =~ /^s3/) {
	print STDERR "Checking availability of s3cmd\n" if $verbose;
	Tools::ensureS3cmd();
} else {
	print STDERR "s3cmd not needed\n" if $verbose;
}
...
if (defined $owner && $push ne "") {
	...
	print STDERR "Creating destination directory and setting owner\n" if $verbose;
...
sub fetch($$$) {
	my ($fname, $url, $md) = @_;
	defined($md) || die;
	print STDERR "Fetching $url $fname $md\n";
	...
	if($fname =~ /\.gz$/) {
		...
		counter("Short read preprocessor,Read data fetched to EC2 (uncompressed),".(-s $newfname));
		counter("Short read preprocessor,Read data fetched to EC2 (un-gzipped),".(-s $newfname));
	} elsif($fname =~ /\.bam$/) {
		ensureSamtools();
		$newfname =~ s/\.bam$/.sam/;
		...
		counter("Short read preprocessor,Read data fetched to EC2 (uncompressed),".(-s $newfname));
		counter("Short read preprocessor,Read data fetched to EC2 (BAM-to-SAM),".(-s $newfname));
	}
	return $newfname;
}
...
sub parseSAM($$) {
	...
	if($labReadGroup && defined($opts{"RG:Z"})) {
		...
		$ws++;
		print STDERR "No read group for read $name\n$samLine\n$_\n";
		die;
		$readGroup = "no-group";
	} else {
		$readGroup = undef;
...
sub doUnpairedUrl($$$$$) {
	...
	while(1) {
		...
		if($labReadGroup) {
			...
			$delayedCounters{"Paired reads with label $readGroup"}++;
		} elsif(defined($lab)) {
			$fullname .= ";LB:$lab";
			$delayedCounters{"Paired reads with label $lab"}++;
		}
		$fullname .= ";$name";
	...
	# Remove input file
	system("rm -f $fn $fn.gz >&2") unless $keep;
	if($push ne "") {
...
sub doPairedUrl($$$$$$$) {
	...
	# Remove input files
	system("rm -f $fn1 $fn1.gz >&2") unless $keep;
	system("rm -f $fn2 $fn2.gz >&2") unless $keep;
	if($push ne "") {
		# Push and remove output files
		pushBatch("${fn1}_$fileno.out");
		system("rm -f ${fn1}_$fileno.out ${fn1}_$fileno.out.* >&2");
...
while (<>) {
	...
	if ($skipfirst) {
		my $trimmed = shift @s;
		print STDERR "-s trimmed \"$trimmed\" from line:\n$_\n";
	}
	...
	if($s[0] =~ /^\s*#/) {  # Skip lines beginning with hash
		...
		print STDERR "Skipping comment line\n";
		next;
	} else {
		print STDERR "Not a comment line\n";
	}
	unless(defined($s[1])) {
		counter("Short read preprocessor,Malformed lines,1");
		print STDERR "Line malformed:\n$_\n";
	...
	if($#s >= 3) {
		...
		print STDERR "Doing paired-end entry ".Util::trim(`basename $url1`)."\n";
		...
		print STDERR "Doing unpaired entry ".Util::trim(`basename $url1`)."\n";
		doUnpairedUrl($url1, $md51, $s[2], urlToFormat($url1), $color);
		counter("Short read preprocessor,Unpaired URLs,1");
	}
	print STDERR "Total unpaired reads: $totunpaired\n";
	print STDERR "Total paired reads: $totpaired\n";
}

counter("Short read preprocessor,Warnings,$ws");
print STDERR "Warnings: $ws\n";
flushDelayedCounters("Short read preprocessor");
