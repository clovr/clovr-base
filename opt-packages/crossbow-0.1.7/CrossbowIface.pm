...
#
sub checkExe($$$$$$) {
	my ($path, $nm, $env, $sub, $arg, $dieOnFail) = @_;
...
$tempdir = "/tmp" unless $tempdir ne "";
my $scriptTempdir = "$tempdir/$app/invoke.scripts";
system("mkdir -p -m 0777 $scriptTempdir");
if(!$hadoopJob && !$localJob) {
	$slaveTempdir = "/mnt/$$" if $slaveTempdir eq "";
} else {
	$slaveTempdir = "$tempdir/$app/$$" if $slaveTempdir eq "";
}
...
$useSamtools = $stages{align} && 0; # && $sampass;
...
if($test) {
	...
	if($localJob || $hadoopJob) {
		...
		$failed = $bowtie eq "" || ($useSamtools && $samtools eq "") || $soapsnp eq ""; 
...
if(!$localJob && !$hadoopJob) {
	...
	my $hadoopStreamingJarBaseName = basename($hadoopStreamingJar);
	$hadoopStreamingJarBaseName =~ /(hadoop-.*).jar/;
	$hadoopVersion = $1;
	$hadoopVersion =~ s/(hadoop|streaming|-|.jar)//g;
...
my $inputAlign = ($preprocess ? $outputPreproc : $input);
my $outputAlign = ($justAlign ? $output : "$intermediate/align");
...
my $info = qq!{
	"startupScripts":[{"name":"s3://b.emr/hadoop-conf", "args":["-s","s3n://$app-emr/conf/conf$cores.xml"]}]}!;
