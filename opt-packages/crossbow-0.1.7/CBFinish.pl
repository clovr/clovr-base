...
GetOptions (
	...
	"cmap:s"          => \$cmap_file,
	"cmapjar:s"       => \$cmap_jar,
	"destdir:s"       => \$dest_dir) || dieusage("Bad option", 1);
...
print STDERR "cmap_file: $cmap_file\n";
print STDERR "cmap_jar: $cmap_jar\n";
print STDERR "local destination dir: $dest_dir\n";
print STDERR "Output dir: $output\n";
...
sub pushResult($) {
	...
	if($output =~ /^s3/i) {
		...
		if($ret != 0) {
			print STDERR "Fatal error: could not put result file $fn into HDFS directory $output\n";
			exit 1;
		}
	} else {
		mkpath($output);
...
if($cmap_jar ne "") {
	print STDERR "Ensuring cmap jar is installed\n";
	...
	print STDERR "Examining extracted files\n";
...
print STDERR "Read $lines lines of output\n";
