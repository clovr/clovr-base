...
##
# Printer that prints to STDERR and, optionally, to a file.
#
my $msgfn = "";
my $msgfh = undef;
sub msg($) {
...
for my $dir (split(/,/, $input)) {
	...
	for my $f (<$dir/*>) {
		...
		while(<F>) {
			chomp;
			my @s = split(/\t/);
			scalar(@s) >= $sortFields || mydie("$sortFields sort fields, but line doesn't have that many:\n$_", 150);
			my $k = join("\t", @s[0..($binFields-1)]);
			$binSizes{$k}++;
		}
...
for(my $i = 0; $i < $nonemptyTasks; $i++) {
	...
	my $cmd;
	msg("Pid $$ sorting task $nfn [".($i+1)." of ".($nonemptyTasks)."]...");
	doSort($i, $externalSort);
	$cmd = cmdifyReducer($i);
...
msg("-- Reduce counters --");
Wrap::getAndPrintCounters($errDir, \&msg);
