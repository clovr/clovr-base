...
##
# Function to call when bowtie exits with non-zero level.
#
sub bowtieError($$) {
	my ($cmd, $lev) = @_;
	# Write the relevant data to an error directory
	mkpath("/tmp/error/Align.pl.$$");
	open ERR, ">/tmp/error/Align.pl.$$/err.sh";
	print ERR "$cmd\n";
	close(ERR);
	Util::run("cp * /tmp/error/Align.pl.$$/");
	Util::run("cp .* /tmp/error/Align.pl.$$/");
	die "$bowtie exited with non-zero exitlevel $lev; see /tmp/error/Align.pl.$$ on ".`hostname`;
}
...
while(<STDIN>) {
	...
	my $len1 = length($altok[1]); $lens{$len1}++;
	my $len2 = 0;
	if($pe) {
		if($discardMate > 0) {
			if($discardMate == 1) {
	...
	if($truncate > 0 && ($len1 > $truncate || $len2 > $truncate)) {
		# Print alignment after truncating it
		my $nlen1 = min($truncate, $len1);
		$truncated++ if $nlen1 < $len1;
		if($pe) {
			my $nlen2 = min($truncate, $len2);
		...
		# Print alignment; no truncation needed
		if($pe) {
			$pass++; $pairedPass++;
			print OUT "r\t$altok[1]\t$altok[2]\t$altok[3]\t$altok[4]\n";
		} else {
			$pass++; $unpairedPass++;
			print OUT "r\t$altok[1]\t$altok[2]\n";
		}
	}
	$records++;
}
