...
sub ensureKeys($$) {
	...
	if($accessKey eq "") {
		...
		} elsif(defined($hadoopHome) && -f "$hadoopHome/conf/hadoop-site.xml") {
			$accessKey = `grep fs.s3n.awsAccessKeyId $hadoopHome/conf/hadoop-site.xml | sed 's/.*<value>//' | sed 's/<\\/value>.*//'`;
			chomp($accessKey);
			if($accessKey eq "") {
				print STDERR "Couldn't get access key from $hadoopHome/conf/hadoop-site.xml\n";
			}
		}
		if($accessKey eq "") {
	...
	if($secretKey eq "") {
		if(defined($ENV{AWS_SECRET_ACCESS_KEY})) {
			...
			$secretKey = `grep fs.s3n.awsSecretAccessKey $hadoopHome/conf/hadoop-site.xml | sed 's/.*<value>//' | sed 's/<\\/value>.*//'`;
			chomp($secretKey);
			if($secretKey eq "") {
				print STDERR "Couldn't get secret key from $hadoopHome/conf/hadoop-site.xml\n";
			}
		}
		if($secretKey eq "") {
