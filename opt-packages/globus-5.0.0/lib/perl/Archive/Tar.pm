...
$VERSION = do { my @a=q$Name: globus_5_0_0 $ =~ /\d+/g; sprintf "%d." . ("%02d" x $#a ),@a };
...
sub _read_tar {
 ...
 READLOOP:
	...
	unless ($extract || ($type ne '' and $type != FILE)) {
	...
	if ($seekable) {
	    $offset += $tar_header_length;
#            print "BEFORE: offset: $offset, type: |$type|\n";
	    $offset += ($size & 0x01ff) ? ($size & ~0x01ff) + 512 : $size
		if $type eq '' or $type == FILE;
#            print "AFTER: offset: $offset, type: |$type|\n";
	}
	$file->gzread ($head, $tar_header_length) 
	    or goto &_drat;
    }
