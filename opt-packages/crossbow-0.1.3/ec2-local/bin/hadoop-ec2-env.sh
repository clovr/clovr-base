...
# BTL: I wish we could use version 0.19.1 or 0.20.0, but they don't
# seem to be installed on any of the publicly-available EC2 images.
# Using either of these newer versions would fix the S3 PUT '/' failure
# problem.
...
# The download URL for the Sun JDK.
# Visit http://java.sun.com/javase/downloads/index.jsp and get the URL for the "Linux self-extracting file".
if [ "$INSTANCE_TYPE" == "m1.small" -o "$INSTANCE_TYPE" == "c1.medium" ]; then
  ...
  JAVA_BINARY_URL='http://cds.sun.com/is-bin/INTERSHOP.enfinity/WFS/CDS-CDS_Developer-Site/en_US/-/USD/VerifyItem-Start/jdk-6u16-linux-i586.bin?BundledLineItemUUID=XJRIBe.pj9YAAAEjtEkMmvcu&OrderID=HLFIBe.pu20AAAEjpkkMmvcu&ProductID=Dx1IBe.prBgAAAEirRcTvuC_&FileName=/jdk-6u16-linux-i586.bin'
else
  ARCH='x86_64'
  BASE_AMI_IMAGE="ami-2a5fba43"  # ec2-public-images/fedora-8-x86_64-base-v1.07.manifest.xml
  JAVA_BINARY_URL='http://cds.sun.com/is-bin/INTERSHOP.enfinity/WFS/CDS-CDS_Developer-Site/en_US/-/USD/VerifyItem-Start/jdk-6u16-linux-x64.bin?BundledLineItemUUID=WI1IBe.pIGsAAAEj82wMmvcG&OrderID=nsxIBe.pf.0AAAEj5WwMmvcG&ProductID=9sJIBe.pnAQAAAEirxcTvuC_&FileName=/jdk-6u16-linux-x64.bin'
fi
...
# BTL: Leaving this uncommented seems to result in the image not being ssh-accessible.
