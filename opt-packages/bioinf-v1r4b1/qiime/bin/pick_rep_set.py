...
script_info['script_usage'].append(("""Simple example: picking a representative set for de novo-picked OTUs""", """The script pick_rep_set.py takes as input an 'OTU map' (via the \"-i\" parameter) which maps OTU identifiers to sequence identifiers. Typically, this will be the output file provided by pick_otus.py. Additionally, a FASTA file is required, via \"-f\", which contains all of the sequences whose identifiers are listed in the OTU map. The following command shows an example where the resulting fasta and log files are written to the directory \"repr_set/\". By default, a representative sequence will be chosen as the most abundant sequence in the OTU. This can be changed to, for example, choose the first sequence listed in each OTU by passing -m first.""", """pick_rep_set.py -i seqs_otus.txt -f seqs.fna -o repr_set/"""))

script_info['script_usage'].append(("""Picking OTUs with "preferred representative" sequences""", """Under some circumstances you may have a fasta file of "preferred representative" sequences. An example of this is if you were to pick OTUs against a reference collection with uclust_ref. In this case you may want your representative sequences to be the sequences from the reference collection, rather than the sequences from your sequencing run. To achieve this, you can pass the original reference collection via -r. If you additionally allowed for new clusters (i.e., sequences which don't match a reference sequence are used as seeds for new OTUs) you'll also need to pass the original sequence collection to pick a representative sequence from the sequencing run in that case.""", """pick_rep_set.py -i seqs_otus.txt -f seqs.fna -r refseqs.fasta -o repr_set/"""))
