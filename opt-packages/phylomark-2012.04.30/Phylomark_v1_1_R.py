...
def tree_loop(fastadir, combined, tree, parallel_workers):
    ...
    def _perform_workflow(data):
        ...
        #subprocess.check_call("mv table_out.txt ./R_output/%s.table.txt" % name,
                              #shell=True)
        os.system("mv %s.pdf ./R_output/%s.plots.pdf" % (name, name))
        #subprocess.check_call("mv Rplots.pdf ./R_output/%s.plots.pdf" % name,
        					  #shell=True)
...
def main(alignment, mask, ref, combined, tree, step_size, frag_length, keep_length, parallel_workers):
    ...
    #subprocess.check_call("rm -rf ./R_output", shell=True)
    #subprocess.check_call("mkdir ./R_output", shell=True)
    try:
