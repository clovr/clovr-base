...
vp-describe-protocols --config-from-protocol=clovr_mugsy \
    ...
    -c pipeline.PIPELINE_NAME=mugsy_${DATE} \
...
TASK_NAME=`vp-run-pipeline --print-task-name --pipeline-name clovr_mugsy_$$_${DATE} --pipeline clovr_wrapper --pipeline-config /tmp/$$.pipeline.conf.${DATE}`
