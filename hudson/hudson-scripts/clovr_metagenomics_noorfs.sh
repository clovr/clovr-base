...
vp-describe-protocols --config-from-protocol=clovr_metagenomics_noorfs \
    ...
    -c input.PIPELINE_NAME=clovr_metagenomics_noorfs_${DATE} \
    > /tmp/$$.pipeline.conf

TASK_NAME=`runPipeline.py --name local  --print-task-name --pipeline-name clovr_metagenomics_noorfs_$$ --pipeline=clovr_wrapper --pipeline-config=/tmp/$$.pipeline.conf`
