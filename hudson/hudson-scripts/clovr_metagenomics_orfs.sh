...
vp-describe-protocols --config-from-protocol=clovr_metagenomics_orfs \
    ...
    -c pipeline.PIPELINE_NAME=clovr_metagenomics_orfs_${DATE} \
    -c cluster.CLUSTER_NAME=$1 \
    -c cluster.CLUSTER_CREDENTIAL=$2 \
    > /tmp/$$.pipeline.conf.${DATE}

TASK_NAME=`vp-run-pipeline --print-task-name --pipeline-name clovr_metagenomics_orfs_$$_${DATE} --pipeline clovr_wrapper --pipeline-config /tmp/$$.pipeline.conf.${DATE}`
