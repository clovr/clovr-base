...
my $DECRYPTION_PKG;
{
    my ($fastq_dump, $fastq_dir) = find_bin("fastq-dump", "optional");
    my ($sam_dump, $sam_dir) = find_bin("sam-dump", "optional");
...
my $HOME_CFG_DIR;
my $fixed_config = 0;
$fixed_config |= do_refseq_cfg() unless ($DECRYPTION_PKG);
$fixed_config |= do_krypto_cfg();
do_schema_cfg() unless ($DECRYPTION_PKG);
